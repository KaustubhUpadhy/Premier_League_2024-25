...
filt = df.loc[df["position"].isin(defenders) & (df['minutes']>=mins)&(df['progressive_passes']>=min_prog_pass)]
#print(filt)

filt["progressive_passes_per90"]=((filt['progressive_passes'] / filt['minutes']) * 90).round(2)
filt["progressive_carries_per90"]=((filt['progressive_carries'] / filt['minutes']) * 90).round(2)
...
for i, idx in enumerate(filt.index):
    ...
    plt.annotate(player_name, (x[idx], y[idx]), fontsize=8,color="silver")
